		...
		if(this.tag == "Player1")
        {
            if(Can_useSkill2 == true)
            {
                if (Input.GetKeyDown(KeyCode.G))
                {
                    ...
                    {
                        GameObject attack = Instantiate(Attack, this.transform.position + new Vector3(1f, 0, 0), Quaternion.identity);
                    ...
                    else
                    {
                        ...
                        GameObject attack = Instantiate(Attack, this.transform.position + new Vector3(-1f, 0, 0), new Quaternion(0, 0, 0, 0));
                        attack.transform.localScale = new Vector3(-2, 2, 0);
                        StartCoroutine("Sk2_time");
                    }
                }
